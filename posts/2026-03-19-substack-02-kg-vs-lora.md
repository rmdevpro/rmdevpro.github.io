...
I was speaking with a fellow engineer who noted that LoRAs could be applied to LLMs in Google Cloud successfully, but was unable to extend the same approach to image generation. It got me thinking about the broader problem. LoRAs are powerful in theory, but in practice they come with significant training overhead and the difficulty of finding the right weights, not to mention that not all models or hosting situations make them practically usable. I've largely moved away from them in favor of knowledge graphs.
...
For real-time teaching — where the system needs to learn good and bad behaviors as they happen — this cycle is too slow and frankly, too nondeterministic. If you burn yourself on the stove a KG can keep you from doing it again pretty easily and quickly, not so easily, quickly or deterministically with a LoRA.
...
The shift from LoRAs to knowledge graphs is really a shift from “retrain the model to know this” to “give the model the right context so it doesn’t need to know this.” One is expensive, slow and limitedly deterministic with small data sets. The other is instant, grows with use and can be highly deterministic with small data sets. There are further implications for how KGs could enhance chain-of-thought reasoning, but that deserves its own discussion.
