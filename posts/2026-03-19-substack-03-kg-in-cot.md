...
But the pattern I am describing is different I think. The existing work is about using KGs to structure reasoning input — giving the model better context to reason with. What I am thinking about is using a KG to correct/augment/guide reasoning output — catching the model mid-chain when it is about to repeat a known mistake, and redirecting it before the mistake propagates through the remaining steps. Or conversely to inject desirable behaviors midstream.

One question for consideration is how the KG acquires its knowledge in the first place — the training signal that tells the graph what worked and what didn't. The answer depends on the domain. For technical systems, the signal can be obvious: logs show mistakes, tests pass or fail, downstream systems break in observable ways. For documented practice, the signal is already written: the policy says otherwise, the architecture says otherwise, the law says otherwise — you encode those constraints directly. But for conversational or subjective domains where the signal comes from user feedback, I would imagine you need a secondary LLM reading elements of the conversation and deciding whether there is a lesson to be learned — extracting the pattern from the interaction and encoding it into the graph. The collection mechanism matters as much as the intervention mechanism. Without it, the graph has nothing to intervene with.

This seems to me to be a powerful way one could use a knowledge graph. Chain-of-thought reasoning is powerful but blind to its own history. The model does not know that it tried this approach yesterday and it failed. A KG does. Connecting the two — not at prompt time, but at each reasoning step — turns CoT from an isolated reasoning exercise into one that learns from experience in real time.
