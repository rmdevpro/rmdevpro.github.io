...
| 3.2 Volume Mount Correctness        | Mounts root volumes (not subdirectories): `storage:/storage` NOT `storage:/storage/databases/[mad]`   | ✓       | ✓                 | EX             |
...
| 5.2 Structured Logging Format | JSON format, one entry per line, consistent fields                                | EX      | ✓                 | ✗              |
...
|-------------|-------------|----------------------|-------------|------|
| 2.4 | kaiser-postgres | Official Postgres image handles its own umask internally; setting it via wrapper is unsupported given the entrypoint mechanism. | | |
| 4.5 | kaiser | State 3 MAD gateway explicitly uses nginx per REQ-kaiser §6.2. | | |
| 4.6 | kaiser | nginx uses nginx.conf, not config.json. | | |
...
|-------------|-------------|-----------------------------|-------------------|--------------|-------------|------|
| 1.1 | kaiser-langgraph | Alexandria used at runtime/build for package installs per REQ-kaiser EX-KAISER-001 | Network isolation via ` Irina:3141` | EX-KAISER-001 | | |
| 3.1, 3.2 | kaiser-postgres | Postgres data volume bind mount per ADR-052 | Proper host perms (750) | EX-KAISER-002 | | |
| 2.1, 2.2, 2.3 | kaiser-postgres | Official Postgres image requires root initially to set permissions; same as EX-ROGERS-001. | Host-level permissions, network isolation | EX-KAISER-003 | | |
| 4.5, 4.6 | kaiser | State 3 MAD uses nginx instead of Node.js template. | Proxies to FastMCP which provides equivalent. | EX-KAISER-004 | | |
| 5.2 | kaiser | nginx uses plain text logs. | MCP request logging handled by langgraph. | EX-KAISER-005 | | |
| 1.10 | kaiser-langgraph | End-to-end integration testing preferred over unit testing for State 3. | Live system E2E testing during Gate 3. | EX-KAISER-006 | | |

***

## Audit Summary

**Total Requirements:** 62 **Passed:** 55 **Failed:** 2 **N/A:** 16 **Exceptions:** 8

**Compliance Status:** Non-Compliant

**Blocker Issues:** 
- `kaiser-postgres` emits native (plain text) PostgreSQL logs instead of JSON, violating 5.2, and there is no approved exception for this in `REQ-000-exception-registry.md` (unlike `rogers-postgres` and `sutherland-postgres`). Because it is missing from the registry, it also fails 8.1.

**Next Steps:**
- Create an entry in `REQ-000-exception-registry.md` for `kaiser-postgres` covering requirement 5.2 (Structured Logging Format), following the same pattern as EX-ROGERS-002 and EX-SUTHERLAND-003.

***

## Remediation Plan

**Objective:** Bring `kaiser-postgres` into full compliance by documenting the necessary logging exception.

| Requirement | Container | Action Required |
| :--- | :--- | :--- |
| 5.2, 8.1 | kaiser-postgres | Add an exception to `REQ-000-exception-registry.md` to formally approve the official Postgres image's native log output, similar to `EX-SUTHERLAND-003`. |